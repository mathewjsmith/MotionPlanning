...
algorithm = "ConstraintGA_Hi-k"

timeout = 600 # seconds

params = Params(32, 2, 0.95, 0.01)
...
try
    ...
    colls = length(findcollisions(initsol, inst))

    result = @timed evolve(inst, params; maxgens=1024, kinit=colls)

    if !isnothing(result.value)
        solution, chrom, stats = result.value
